...
class TowerModule(AnsibleModule):
    url = None
    AUTH_ARGSPEC = dict(
        tower_host=dict(required=False, fallback=(env_fallback, ['SSC_TOWER_URL','AWX_HOST','TOWER_HOST'])),
        tower_username=dict(required=False, fallback=(env_fallback, ['AWX_USERNAME','TOWER_USERNAME'])),
        tower_password=dict(no_log=True, required=False, fallback=(env_fallback, ['AWX_PASSWORD','TOWER_PASSWORD'])),
        validate_certs=dict(type='bool', aliases=['tower_verify_ssl'], required=False, fallback=(env_fallback, ['AWX_VERIFY_SSL','TOWER_VERIFY_SSL'])),
        tower_oauthtoken=dict(type='raw', no_log=True, required=False, fallback=(env_fallback, ['AWX_OAUTH_TOKEN','TOWER_OAUTH_TOKEN'])),
