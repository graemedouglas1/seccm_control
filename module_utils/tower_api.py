...
class TowerAPIModule(TowerModule):
    ...
    def __init__(self, argument_spec, direct_params=None, error_callback=None, warn_callback=None, **kwargs):
        ...
        self.retries = Retry(total=10, raise_on_status=False, backoff_factor=1, status_forcelist=[429,500,502,503,504])
        self.timeout = Timeout(connect=2.0, read=None)
        self.http    = urllib3.PoolManager(cert_reqs='CERT_NONE', assert_hostname=False, retries=self.retries, timeout=self.timeout)
    ...
    def revert_branch(self, scm_branch, url, *args, **kwargs):
        data = {
           'scm_branch': scm_branch,
           'update_project': False
        }
        return self.patch_endpoint(url, **{'data': data})
    ...
    def make_request(self, method, endpoint, *args, **kwargs):
        ...
        if not self.oauth_token and not self.authenticated and not self.basic_authentication:
            # This method will set a cookie in the cookie jar for us and also an oauth_token
            self.authenticate(**kwargs)

        if self.basic_authentication:
            # if we want to use basic authentication, set base64 string of user credentials
            userpass = self.username + ':' + self.password
            encoded_u = base64.b64encode(userpass.encode()).decode()
            self.headers['Authorization'] = 'Basic {0}'.format(encoded_u)
        ...
        try:      
            response = self.http.request(method=method, url=url.geturl(), headers=self.headers, redirect=True, body=data) 
        ...
        except(urllib3.exceptions.ConnectionError) as con_err:
            ...
            if response.status >= 500:
                self.fail_json(msg='The host sent back a server error ({1}): {0}. Please check the logs and try again later'.format(url.path, errh))
            ...
            # Sanity check: Did we get some other kind of error?  If so, write an appropriate error message.
                ...
                try:
                    return {'status_code': response.status, 'json': loads(page_data)}
                # JSONDecodeError only available on Python 3.5+
                except ValueError:
                    return {'status_code': response.status, 'text': page_data}
            elif response.status == 204 and method == 'DELETE':
                # A 204 is a normal response for a delete function
                pass
            else:
                self.fail_json(msg="Unexpected return code when calling {0}: {1}".format(url.geturl(), errh))
        except(Exception) as e:
            self.fail_json(msg="There was an unknown error when trying to connect to {2}: {0} {1}".format(type(e).__name__, e, url.geturl()))

        tower_type = response.headers['X-API-Product-Name']
        tower_version = response.headers['X-API-Product-Version']
        ...
        try:
            return {'tower_type': tower_type, 'status_code': status_code, 'json': response_json}
        except:
    ...
    def authenticate(self, **kwargs):
        if self.username and self.password:
            ...
            try:
                response = self.http.request(method='POST', url=api_token_url, headers=self.headers, redirect=True, body=dumps(login_data)) 
    ...
    def create_if_needed(self, existing_item, new_item, endpoint, on_create=None, item_type='unknown', associations=None, return_json=None):
    ...
    def update_if_needed(self, existing_item, new_item, on_update=None, associations=None, return_json=None):
        # This will exit from the module on its own
        ...
        # Setting project branch to master_branch value (Post Update)
        if ('scm_branch' in new_item and 'master_branch' in new_item):
            if (new_item['scm_branch'] != new_item['master_branch']):
                status = self.revert_branch(
                    scm_branch=new_item['master_branch'],
                    url=item_url
                )
    ...
    def create_or_update_if_needed(self, existing_item, new_item, endpoint=None, item_type='unknown', on_create=None, on_update=None, associations=None, return_json=None):
        if existing_item:
            return self.update_if_needed(existing_item, new_item, on_update=on_update, associations=associations, return_json=return_json)
        else:
            return self.create_if_needed(existing_item, new_item, endpoint, on_create=on_create, item_type=item_type, associations=associations, return_json=return_json)
    ...
    def logout(self):
        if self.authenticated and self.oauth_token_id:
            ...
            try:
                response = self.http.request(method='DELETE', url=api_token_url, headers=self.headers, redirect=True) 
